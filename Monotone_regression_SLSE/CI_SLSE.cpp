...
List CI_SLSE(NumericMatrix X, int N, int seed1)
{
    ...
    data_bootstrap0 = new double[n+1];
    ...
    for (iter=0;iter<NumIt;iter++)
    {
        ...
        for (i=1;i<=n;i++)
            data_bootstrap0[i]=bootstrap_data[i][0];
    ...
    List out = List::create(Rcpp::Named("CI_SLSE")=out1,Rcpp::Named("data")=out2);
    // free memory
    ...
    delete[] cumw; delete[] cs; delete[] ff;  delete[] residu;
    
    delete[] tt; delete[] pp; delete[] tt_bootstrap; delete[] pp_bootstrap;
    delete[] ff_bootstrap; delete[] SLSE; delete[] SLSE_data; delete[] SLSE2;
