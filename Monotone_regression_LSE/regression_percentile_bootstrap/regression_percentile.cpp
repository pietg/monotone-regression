//
//  Computation of percentile intervals for monotone regression
//
...
void    data_bootstrap_percentile(int J, int nJ[], double ybar[],
                                  double **bootstrap_data,double sigma, int seed);
double  f0(double x);
...
List regression_percentile(int N)
{
    int     n,ngrid,i,j,k,J,*nJ,seed;
    int     iter,iter2,NumIt,NumIt2;
    int     percentile1,percentile2,*percentage;
    double  *lowbound,*upbound,**f3,*f4;
    double  **data,**bootstrap_data,*LSE;
    double  *cumw,*cs, *ybar,*ybar_bootstrap,sigma;
    double  *grid,*test;
    double  *theta_bootstrap,*theta_reduced,*theta_monotone;
    ...
    nJ =    new int[J+1];
    ybar =  new double[J+1];
    ybar_bootstrap =  new double[J+1];
    theta_reduced =  new double[J+1];
    theta_monotone =  new double[J+1];
    theta_bootstrap =  new double[J+1];
    ...
    for (iter=0;iter<NumIt;iter++)
    {
        ...
        for (i=1;i<=n;i++)
        {
            for (j=1;j<=J;j++)
            {
                if (data[i][0]>(double)(j-1)/J && data[i][0]<=(double)j/J)
                sigma += SQR(data[i][1]-ybar[j]);
            }
        }
        ...
        for (iter2=0;iter2<NumIt2;iter2++)
        {
            ...
            data_bootstrap_percentile(J,nJ,ybar,bootstrap_data,sigma,seed);
            
            for (j=1;j<=J;j++)
                ybar_bootstrap[j]=bootstrap_data[j][1];
                
            compute_LSE(J,nJ,cumw,cs,ybar_bootstrap,theta_reduced,theta_bootstrap);
    ...
    delete[] cumw; delete[] cs; delete[] ybar; delete[] ybar_bootstrap;
    
    delete[] LSE; delete[] nJ;
    delete[] theta_monotone; delete[] theta_bootstrap; delete[] theta_reduced; delete[] test;
...
void data_bootstrap_percentile(int J, int nJ[], double ybar[], double **bootstrap_data, double sigma, int seed)
{
    int    j;
    
    std::mt19937_64 gen(seed);
    std::normal_distribution<double> dis_normal(0,1);
    
    for (j=1;j<=J;j++)
    {
        bootstrap_data[j][0] = (double)j/J;
        bootstrap_data[j][1] = ybar[j]+(sigma/sqrt(nJ[j]))*dis_normal(gen);
    }
}
